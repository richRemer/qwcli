...
State.prototype.expand = function(num, expansion) {
    if (arguments.length === 1) expansion = num, num = 1;

    var args,
        expanded = null;

    if (this.index + num <= this.args.length) {
        args = this.args.slice(0, this.index)
            .concat(expansion)
            .concat(this.args.slice(this.index + num));

        expanded = this.update({args: freeze(args)});
    }
